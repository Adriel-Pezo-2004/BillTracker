...
function getCurrentPaymentCycle() {
  ...
  console.log("[v0] Fecha actual:", today.toISOString(), "Día:", currentDay)

  if (currentDay <= 10) {
    // Estamos en el período de pago del ciclo anterior (del 1 al 10)
    ...
    console.log("[v0] Período de pago activo, ciclo:", cycle)
    return cycle
  } else {
    // Estamos acumulando gastos para el próximo ciclo (del 11 al final del mes)
    const cycle = {
    ...
    console.log("[v0] Período de acumulación activo, ciclo:", cycle)
    return cycle
  }
}

// Función para verificar si debemos mostrar el total de tarjetas
function shouldShowCreditCardTotal() {
  const cycle = getCurrentPaymentCycle()
  console.log("[v0] ¿Mostrar total de tarjeta?", cycle.isPaymentPeriod)

  return true
}

function getContableMonth(date: Date) {
  // Si el día es menor o igual a 10, pertenece al mes anterior para efectos contables
  if (date.getDate() <= 10) {
...
// Función para verificar si una fecha de tarjeta pertenece al ciclo actual
...
// Función para filtrar tarjetas por ciclo de pago
function filterCreditCardByCycle(tarjetas: any[]) {
  ...
  const filteredCards = tarjetas.filter((tarjeta) => {
    ...
    console.log("[v0] Tarjeta:", format(tarjetaDate, "dd/MM/yyyy"), "¿Pertenece al ciclo?", belongs)
...
export default function Dashboard() {
  ...
  // Información del ciclo de pago y lógica de filtrado
  ...
  const tarjetasFiltradas = useMemo(() => {
    if (paymentCycle.isPaymentPeriod) {
      // Durante período de pago (días 1-10): mostrar tarjetas del ciclo anterior
      return filterCreditCardByCycle(tarjetaCredito)
    } else {
      // Durante período de acumulación (días 11-fin): mostrar tarjetas del ciclo actual
      const today = new Date()
      ...
      return tarjetaCredito.filter((tarjeta) => {
        ...
        // Incluir tarjetas del mes actual después del día 10
              ...
              <CardDescription className="text-center text-primary-foreground/80">
                {mostrarTotalTarjeta ? (
                  <span className="text-orange-200">
                    📅 Período de pago activo - Ciclo {paymentCycle.month}/{paymentCycle.year}(
                    {paymentCycle.daysRemaining} días restantes)
                  </span>
                ) : (
                  <span className="text-primary-foreground/60">
                    🔒 Período de acumulación - Ciclo {paymentCycle.month}/{paymentCycle.year}
                  </span>
                )}
              </CardDescription>
            </CardHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="shadow">
              ...
              <CardContent>
                <div className="flex justify-between items-center mb-4">
                  <span className="text-sm text-muted-foreground">Últimos {ingresosMostrados.length}</span>
                  {ingresos.length > 5 && (
                    <Button
                      variant="outline"
                      size="sm"
          ...
          >
            <Card className="shadow">
              ...
              <CardContent>
                <div className="flex justify-between items-center mb-4">
                  <span className="text-sm text-muted-foreground">Últimos {gastosMostrados.length}</span>
                  {gastos.length > 5 && (
                    <Button
                      variant="outline"
                      size="sm"
          ...
          >
            <Card className="shadow">
              ...
              <CardContent>
                <div className="flex justify-between items-center mb-4">
                  <span className="text-sm text-muted-foreground">Últimos {tarjetaMostrados.length}</span>
